...
import (
	...
	"github.com/duolabmeng6/goefun/src/gabs"
)
...
func (this *EJson) GetString(key string) string {
	if this.Json.ExistsP(key) {
		...
		return ecore.E到文本(this.Json.Path(key).Data())
	}
	return ""
}
...
func (this *EJson) GetInt(key string) int64 {
	if this.Json.ExistsP(key) {
		value := ecore.E到整数(this.Json.Path(key).Data())
		return value
	}
	return 0
}
...
func (this *EJson) LoadFromJsonString(data string) bool {
	var err error
	this.Json, err = gabs.ParseJSON(ecore.E到字节集(data))
	//fmt.Print(err)
	return err != nil
}
