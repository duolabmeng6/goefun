...
func Test_取现行时间(t *testing.T) {
	...
	//t.Log("E到友好时间", time4.E到友好时间())

	t.Log("E到时间从时间戳", E到时间从时间戳(E取现行时间().E取时间戳()).E时间到文本(""))

	t.Log("E取时间戳", E取现行时间().E取时间戳())

	//time2 = E到时间("2019-02-04 22:53:02")
	//t.Log("E到友好时间", time2.E到友好时间())
	time2 = E取现行时间().E增减日期(0, 0, -10)
	t.Log("E到友好时间", time2.E到友好时间(), time2.E时间到文本("Y-m-d H:i:s"))
}
