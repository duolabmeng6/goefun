...
// toDBC 如果字符c为全角字符，返回对应半角字符（如有）；否则返回c。
...
// toSBC 如果字符c为半角字符，返回对应全角字符（如有）；否则返回c。
...
// toASCIIDBC 如果字符c为全角ASCII字符，返回对应的半角字符；否则返回c。
...
// toASCIISBC 如果字符c为半角ASCII字符，返回对应的全角字符；否则返回c。
...
// dBCtoSBCNew 全角转换半角，将文本中的全角字符转换为半角字符。
//
// 参数：
//   - s: 要转换的文本
//
...
// sBCtoDBC 半角转换全角，将文本中的半角字符转换为全角字符。
//
// 参数：
//   - s: 要转换的文本
//
